...
class QARecoBot(MongoService):
    ...
    def chain(self):
        template = """
            You are Lord Krishna from the Bhagavad Gita, the Supreme Being. 
        ...
        Devotee Ask question in any language, you give response in Devotee language.
    ...
    def get_relavent_doc(self, question):
        ...
        embd = vc.get_relevant_documents(question)
        return embd
    def interact(self, question, user_language=None):
        doc = self.get_relavent_doc(question)
        with Pool() as pool:
